if (localStorage.name && localStorage.email)  {
  ...
  $('input[name="entry.583196713"]').val(localStorage.name);
...
$(function() {
  $("[name=send]").click(function (e) {
   ...
   var name_tl = localStorage.name = form.find('input[name="entry.583196713"]').val();
   ...
   if (!(error == 1)) {
    $(send_btn).each(function() {
      ...
      $.ajax({
       ...
       $.ajax({
        type: 'POST',
        url: 'https://docs.google.com/forms/d/e/1FAIpQLScxNCFpM0-YxR-n4NEOR5EC_qBEkqxq9IDUgHJkw0opDvqaQA/formResponse',
        dataType: 'json',
        data: msg,
      });
      // Отправка в Telegramm
       $.ajax({
         type: 'POST',
         url: 'https://api.telegram.org/bot357106214:AAHodgTmo25b66zg0hgGIx_gmsBvpyHSvdk/sendMessage?chat_id=-183516110&text=У вас новый лид c Rezart Article мой господин: ' + name_tl + ' | ' + email_tl + ' | ' + phone_tl + ' Продайте ему что-нибудь поскорее!'
         // data: short_msg,
       });
    // Отправка на почту
    $.ajax({
      ...
      success: function() {
        ...
        window.location = 'http://rezart.agency/am/success/';
        // dataLayer.push({
        //   'form_type': formType,
        //   'event': "form_submit"
        // });
